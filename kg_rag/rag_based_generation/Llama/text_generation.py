...
LLM_MODEL_CACHE_DIR = config_data["LLM_MODEL_CACHE_DIR"]
HF_TOKEN = config_data["HF_TOKEN"]


INSTRUCTION = "Context:\n\n{context} \n\nQuestion: {question}"

vectorstore = load_chroma(VECTOR_DB_PATH, SENTENCE_EMBEDDING_MODEL_FOR_NODE_RETRIEVAL, cache_folder=LLM_MODEL_CACHE_DIR)
embedding_function_for_context_retrieval = load_sentence_transformer(SENTENCE_EMBEDDING_MODEL_FOR_CONTEXT_RETRIEVAL, cache_folder=LLM_MODEL_CACHE_DIR)
node_context_df = pd.read_csv(NODE_CONTEXT_PATH)

def main():
    print(" ")
    # question = input("Enter your question : ")
    question = "What is the gene associated with hypochondrogenesis?"
    if not INTERACTIVE:
        template = get_prompt(INSTRUCTION, SYSTEM_PROMPT)
        prompt = PromptTemplate(template=template, input_variables=["context", "question"])
        llm = llama_model(MODEL_NAME, BRANCH_NAME, CACHE_DIR, hf_token=HF_TOKEN, stream=True, method=METHOD)
